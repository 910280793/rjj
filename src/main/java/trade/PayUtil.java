...
public class PayUtil {
    ...
    // 测试当面付2.0生成支付二维码
    public String test_trade_precreate(OrderVo order, HttpServletRequest request) {
        String name="";
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        ...
        String body = "共"+order.getPayment()+"元";
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String p=path+"/%s.png";
                 name=response.getOutTradeNo();
                String filePath = String.format(p, name);
